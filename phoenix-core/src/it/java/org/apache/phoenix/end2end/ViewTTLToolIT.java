...
public class ViewTTLToolIT extends ParallelStatsDisabledIT {
    ...
    private void alterPhoenixTTL(Connection conn, String viewName, long phoenixTTLValue)
            throws SQLException {
        try (Statement stmt = conn.createStatement()) {
            stmt.execute(
                    String.format("ALTER VIEW %s SET PHOENIX_TTL= %d", viewName, phoenixTTLValue));
        }
    }

    private void createTenantViewOnGlobalTableAndUpsertData(Connection conn, String globalView, String viewName,
                                         long phoenixTTLValue) throws SQLException {
        String ddl = "CREATE VIEW " + viewName + " (PK1 BIGINT NOT NULL, PK2 BIGINT NOT NULL, PK3 BIGINT, PK4 BIGINT" +
                " CONSTRAINT PKVIEW PRIMARY KEY(PK1,PK2)) AS SELECT * FROM " + globalView +
                " PHOENIX_TTL=" + phoenixTTLValue;
        ...
        for (int i = 0; i < NUMBER_OF_UPSERT_ROWS; i++) {
            try( PreparedStatement pStmt = conn.prepareStatement(
                    "UPSERT INTO " + viewName + " (PK1,PK2,PK3,PK4) VALUES(?,?,?,?)")) {
                pStmt.setInt(1, i);
                pStmt.setInt(2, i);
                pStmt.setInt(3, i);
                pStmt.setInt(4, i);
    ...
    private void createViewAndUpsertData(Connection conn, String tableName, String viewName,
                                         long phoenixTTLValue, String whereClause) throws SQLException {

        String ddl = "CREATE VIEW " + viewName +
                " (PK1 BIGINT NOT NULL, PK2 BIGINT NOT NULL, PK3 BIGINT, PK4 BIGINT CONSTRAINT PKVIEW PRIMARY KEY(PK1,PK2)) " +
                "AS SELECT * FROM " + tableName + " " + whereClause + " PHOENIX_TTL=" + phoenixTTLValue;

        if (whereClause == null) {
            ddl = "CREATE VIEW " + viewName +
                    " (PK1 BIGINT NOT NULL, PK2 BIGINT NOT NULL, PK3 BIGINT, PK4 BIGINT CONSTRAINT PKVIEW PRIMARY KEY(PK1,PK2)) " +
                    "AS SELECT * FROM " + tableName + " PHOENIX_TTL=" + phoenixTTLValue;
        }
        ...
        if (whereClause == null) {
            insertDataForTableWithoutWhereClause(conn, viewName);
        } else {
            insertDataForTableWithWhereClause(conn, viewName);
        }
        conn.commit();
    }

    private void insertDataForTableWithWhereClause(Connection conn, String viewName) throws SQLException {
        for (int i = 0; i < NUMBER_OF_UPSERT_ROWS; i++) {
    ...
    private void insertDataForTableWithoutWhereClause(Connection conn, String viewName) throws SQLException {
        for (int i = 0; i < NUMBER_OF_UPSERT_ROWS; i++) {
            try (PreparedStatement stmt = conn.prepareStatement(
                    "UPSERT INTO " + viewName + " (PK1,PK2,PK3,PK4,ID) VALUES(?,?,?,?,?)")) {
                stmt.setInt(1, i);
                stmt.setInt(2, i);
                stmt.setInt(3, i);
                stmt.setInt(4, i);
    ...
    private void createViewOnGlobalViewAndUpsertData(Connection conn, String tableName, String viewName,
                                                     long phoenixTTLValue) throws SQLException {
        String ddl = "CREATE VIEW " + viewName + " AS SELECT * FROM " + tableName + " PHOENIX_TTL= " + phoenixTTLValue;
    ...
    public void testViewOnMultiTenantTable() throws Exception {
        ...
        try (Connection globalConn = DriverManager.getConnection(getUrl());
            ...
            createViewAndUpsertData(tenant1Connection, fullTableName, fullViewName1, PHOENIX_TTL_EXPIRE_IN_A_MILLISECOND, null);
            createViewAndUpsertData(tenant2Connection, fullTableName, fullViewName2, PHOENIX_TTL_EXPIRE_IN_A_DAY, null);
    ...
    public void testTenantViewOnMultiTenantTableCases() throws Exception {
        ...
        try (Connection globalConn = DriverManager.getConnection(getUrl());
            ...
            alterPhoenixTTL(tenant2Connection, fullViewName2, PHOENIX_TTL_EXPIRE_IN_A_MILLISECOND);
    ...
    public void testAllViewCases() throws Exception {
        ...
        try (Connection globalConn = DriverManager.getConnection(getUrl());
            ...
            // modify PHOENIX_TTL time and rerun the MR job for the table level.
            // it should only delete views on
            alterPhoenixTTL(tenant2Connection, schema1 + "." + viewName1, PHOENIX_TTL_EXPIRE_IN_A_MILLISECOND);
            alterPhoenixTTL(tenant1Connection, schema1 + "." + viewName2, PHOENIX_TTL_EXPIRE_IN_A_MILLISECOND);
            alterPhoenixTTL(tenant2Connection, schema1 + "." + viewName2, PHOENIX_TTL_EXPIRE_IN_A_MILLISECOND);
            alterPhoenixTTL(globalConn, viewName2, PHOENIX_TTL_EXPIRE_IN_A_MILLISECOND);
    ...
    public void testDeleteExpiredRowsWForMultiTenantOnAnIndexTableCase() throws Exception {
        ...
        try (Connection globalConn = DriverManager.getConnection(getUrl());
            ...
            createViewAndUpsertData(tenant1Connection, baseTable, viewName1, PHOENIX_TTL_EXPIRE_IN_A_DAY, null);
            createViewAndUpsertData(tenant2Connection, baseTable, viewName2, PHOENIX_TTL_EXPIRE_IN_A_DAY, null);
            ...
            // alter the view ttl and all rows should expired immediately.
            alterPhoenixTTL(tenant2Connection, viewName2, PHOENIX_TTL_EXPIRE_IN_A_MILLISECOND);
    ...
    public void testDeleteExpiredRowsForATenantHasMultiViewsOnAnIndexTableCase() throws Exception {
        ...
        try (Connection globalConn = DriverManager.getConnection(getUrl());
            ...
            ddl = "CREATE VIEW " + viewName1 + " AS SELECT * FROM " + baseTable + " WHERE ID = 'A' PHOENIX_TTL= "
                    + PHOENIX_TTL_EXPIRE_IN_A_DAY;
            tenantConnection.createStatement().execute(ddl);

            ddl = "CREATE VIEW " + viewName2 + " AS SELECT * FROM " + baseTable + " WHERE ID = 'B' PHOENIX_TTL= "
                    + PHOENIX_TTL_EXPIRE_IN_A_DAY;
            tenantConnection.createStatement().execute(ddl);
            ...
            alterPhoenixTTL(tenantConnection, viewName1, PHOENIX_TTL_EXPIRE_IN_A_MILLISECOND);
    ...
    @Test
    public void testViewWithWhereClause() throws Exception {
        String baseTable = generateUniqueName();
        String indexTable = "_IDX_" + baseTable;
        String tenant1 = generateUniqueName();
        String tenant2 = generateUniqueName();
        ...
        try (Connection globalConn = DriverManager.getConnection(getUrl());
            ...
            createMultiTenantTable(globalConn, baseTable);
            createViewAndUpsertData(
                    tenant1Connection, baseTable, viewName1, PHOENIX_TTL_EXPIRE_IN_A_DAY, " WHERE ID ='V01' ");
            createViewAndUpsertData(
                    tenant2Connection, baseTable, viewName2, PHOENIX_TTL_EXPIRE_IN_A_DAY, " WHERE ID ='V02' ");
            ...
            verifyNumberOfRows(baseTable, tenant1, NUMBER_OF_UPSERT_ROWS, globalConn);
            verifyNumberOfRows(baseTable, tenant2, 0, globalConn);
        }
    }

    @Test
    public void testViewWithWhereClauseWithIndex() throws Exception {
        String baseTable = generateUniqueName();
        String indexTable = "_IDX_" + baseTable;
        String tenant1 = generateUniqueName();
        String tenant2 = generateUniqueName();
        ...
        try (Connection globalConn = DriverManager.getConnection(getUrl());
            ...
            tenant1Connection.createStatement().execute(
                    "CREATE INDEX " + indexView1 + " ON " + viewName1 + "(PK3) INCLUDE (PK4)");

            tenant2Connection.createStatement().execute(
                    "CREATE INDEX " + indexView2 + " ON " + viewName2 + "(PK3) INCLUDE (PK4)");
    ...
    @Test
    public void testTenantViewOnGlobalViewWithWhereClause() throws Exception {
        String baseTable = generateUniqueName();
        String globalViewName = generateUniqueName();
        String tenant1 = generateUniqueName();
        String tenant2 = generateUniqueName();
        ...
        try (Connection globalConn = DriverManager.getConnection(getUrl());
            ...
            createMultiTenantTable(globalConn, baseTable);
            String ddl = "CREATE VIEW " + globalViewName + "  AS SELECT * FROM " + baseTable + " WHERE ID ='V01'";
            globalConn.createStatement().execute(ddl);
    ...
    @Test
    public void testTenantViewOnGlobalViewWithWhereClauseWithIndex() throws Exception {
        String baseTable = generateUniqueName();
        String indexTable = "_IDX_" + baseTable;
        String tenant1 = generateUniqueName();
        String tenant2 = generateUniqueName();
        ...
        try (Connection globalConn = DriverManager.getConnection(getUrl());
            ...
            createTenantViewOnGlobalTableAndUpsertData(
                    tenant1Connection, globalViewName, viewName1, PHOENIX_TTL_EXPIRE_IN_A_DAY);
            createTenantViewOnGlobalTableAndUpsertData(
                    tenant2Connection, globalViewName, viewName2, PHOENIX_TTL_EXPIRE_IN_A_DAY);
