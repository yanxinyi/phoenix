...
public class FindViewCorruptionTool extends Configured implements Tool {
    ...
    private void populateViewSetMetadata(PhoenixConnection phoenixConnection) throws Exception {
        ResultSet viewRS = phoenixConnection.createStatement().executeQuery(VIEW_QUERY);
        while (viewRS.next()) {
    ...
    private void parseOptions(String[] args) throws Exception {
        ...
        if (cmdLine.hasOption(TENANT_ID_OPTION.getOpt()) && TENANT_ID_OPTION.getArgName()== null) {
            throw new IllegalStateException(TENANT_ID_OPTION.getLongOpt() + " requires argument.");
        }
        if (cmdLine.hasOption(SCHEMA_OPTION.getOpt()) && SCHEMA_OPTION.getArgName()== null) {
            throw new IllegalStateException(SCHEMA_OPTION.getLongOpt() + " requires argument.");
        }
        if (cmdLine.hasOption(TABLE_OPTION.getOpt()) && TABLE_OPTION.getArgName()== null) {
            throw new IllegalStateException(TABLE_OPTION.getLongOpt() + " requires argument.");
        }
    ...
    private void writeCorruptedViews() throws IOException {
        ...
        try (FileWriter writer = new FileWriter(outputPath + fileName)) {
            ...
            for (View view : this.corruptedViewSet) {
                bw.write(view.getTenantId() + "," + view.getSchemaName() + "," +
