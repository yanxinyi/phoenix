...
public class ViewTTLTool extends Configured implements Tool {
    ...
    private Map<String, List<ViewInfo>> map = new HashMap<>();
    private List<ViewInfo> views = new ArrayList<>();
    Configuration configuration;
    Connection connection;


    public class MultiViewTTLTool extends Configured implements Tool {
        ...
        MultiViewTTLTool(String baseTableName, List<ViewInfo> viewInfoList) {
        ...
        private void configureJob() throws Exception {
            this.job = Job.getInstance(getConf(),
                    "ViewTTLTool-" + baseTableName + "-" + System.currentTimeMillis());
            PhoenixMapReduceUtil.setMultiViewInput(this.viewInfoList, job, ViewTTLTestWritable.class);

//            PhoenixConfigurationUtil.setMRJobType(job.getConfiguration(), PhoenixConfigurationUtil.MRJobType.VIEW_TTL_DELETE);

            job.setJarByClass(MultiViewTTLTool.class);
...
//                    Gauge.class, MetricRegistriesImpl.class);
    ...
    private void getAllViewTTLList(Connection phoenixConnection) throws SQLException {
        ...
        while (viewRS.next()) {
            ViewInfo view = new ViewInfo(viewRS.getString(1), viewRS.getString(2),
                    viewRS.getLong(3));
            views.add(view);
        }
...
//        }
    ...
    private void runDeleteViewTTLJobs() throws Exception {
        ...
        MultiViewTTLTool tool = new MultiViewTTLTool("", views);
        for (ViewInfo view : views) {
            if (sb.length() != 0) {
                sb.append(ViewInfo.VIEW_INFO_SERIALIZED_SPLITTER);
            }
            sb.append(view.serialized());
        }
...
//        viewNames = viewNames.substring(0, viewNames.length() - 1);
        ...
        config.set(PhoenixConfigurationUtil.MAPREDUCE_VIEW_TTL_INFO_LIST, sb.toString());
