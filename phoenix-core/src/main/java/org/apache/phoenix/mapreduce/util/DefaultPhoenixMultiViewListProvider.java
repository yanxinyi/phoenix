...
public class DefaultPhoenixMultiViewListProvider implements PhoenixMultiViewListProvider {
    ...
    public List<ViewInfoWritable> getPhoenixMultiViewList(Configuration configuration) {
        ...
        boolean isQueryMore =
                configuration.get(PhoenixConfigurationUtil.MAPREDUCE_PHOENIX_TTL_DELETE_JOB_ALL_VIEWS) != null;
        ...
        try (PhoenixConnection connection = (PhoenixConnection) ConnectionUtil.getInputConnection(configuration)){
            Table catalogOrChildTable = getCatalogOrChildTable(connection, configuration);
    ...
    private Table getCatalogOrChildTable(PhoenixConnection connection, Configuration conf) throws Exception {
        TableName catalogOrChildTableName = ViewUtil.getSystemTableForChildLinks(0, conf);
        return connection.getQueryServices()
