...
public class PhoenixMultiInputUtil {
    ...
    public static Connection buildTenantConnectionWithSpecialProperty(String url, String tenantId, String attribute) throws SQLException {
        Properties props = new Properties();
        props.setProperty(PhoenixRuntime.TENANT_ID_ATTRIB, tenantId);
        props.setProperty(attribute, "false");
        return DriverManager.getConnection(url, props);
    }
    ...
    public static String getFetchViewQuery(Configuration configuration) {
        ...
        } else if ((configuration.get(PhoenixConfigurationUtil.MAPREDUCE_PHOENIX_TTL_DELETE_JOB_PER_TABLE) != null)) {
            query = PhoenixMultiInputUtil.constructViewMetadataQueryBasedOnTable(
                    configuration.get(PhoenixConfigurationUtil.MAPREDUCE_PHOENIX_TTL_DELETE_JOB_PER_TABLE));
